...
/*
    Given
        - 테스트 유저를 생성, jjwt 라이브러리를 이용해 리프레시 톸큰을 만들어 데이터베이스에 저장. 토큰 생성 API의 요청 본문에 토큰을 포함하여
            요청 객체를 생성
    When
        - 토큰 추가 API에 요청을 ㅗ밴ㅂ니다. 이때 요청 타입은 JSON이며, given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보냅니다.
   Then
        - 응답 코드가 201 Created인지 화깅ㄴ하고 응답으로 온 액세스 토큰이 비어 있지 않은지 확인
 */
...
class TokenApiControllerTest {
    ...
    public void createNewAccessToken() throws Exception {
        ...
        resultActions
                ...
                // import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.json.Path;
